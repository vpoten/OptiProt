 ...
 * @author victor
 */
public class RotamerMutationOperator extends MutationOperator {

    public RotamerMutationOperator(final Configuration a_config)
    ...
    public void operate(final Population a_population,
        ...
        for (int i = 0; i < size; i++) {
          ...
          if (mutate) {
            ...
            mutateGene(genes[target], target, generator);
            
          }
        }
      }

    private void mutateGene(final Gene a_gene, final int target,
            final RandomGenerator a_generator) {

        a_gene.applyMutation( target, a_generator.nextDouble() );
    }
